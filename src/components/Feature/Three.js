...
export default function SingleFeatureThree(props) {
  const { id, featureClass, Icon, Title, Description, btnText, Slug } = props;

  // Define custom link logic
  const getLink = () => {
    if (id === 4) return "/mandatorydisclosure"; // 👈 Your custom route
    ...
    <div className={featureClass ? featureClass : 'it-feature-item text-center'}>
      ...
      <div className="it-feature-item-content z-index">
        ...
        <div className="it-feature-text pt-30">
          <h4 className="it-feature-title">{Title || 'oooo'}</h4>
          <p>{Description || 'kk'}</p>
        </div>
        <div className="it-feature-button">
          <Link href={getLink()} className="it-btn-border">
            <span>
              {btnText || 'Learn More'}
